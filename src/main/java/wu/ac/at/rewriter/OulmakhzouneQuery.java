...
public class OulmakhzouneQuery implements RDFQuery{
	...
	public OulmakhzouneQuery(String q) throws Exception{
...
//  SK ISWC Update don't make them optional    	            		
	private List<Element> addOptional(List<Element> list, Element e, Quad auth, Node graph) throws Exception{
					...
					if(matchedQuads != null){
						...
						for (Match m : matchedQuads){
							if(m.exact == false){
								...
								list2.remove(m.index);	
								if(list2.isEmpty())
									removals.add(index-1);
							
								addOptional(list2, e2, auth, graph); 							
						    	...
						    	if(e2 instanceof ElementPathBlock){
						    		...
						    		for(TriplePath path : pathBlock.getPattern().getList()){
						            	...
						            	if(path.isTriple()){
						            		...
						            		//not equals or
						            		addNotEqualsOrQuadFilter(((ElementGroup) namedGraph), auth, q);
						            	}
						    		}
						    	}
